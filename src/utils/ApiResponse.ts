...
abstract class ApiResponse {
    ...
    //@ts-ignore
    protected async prepare<T extends ApiResponse>(res: any, response: T): any {
        console.log(res.logId)
        await logsModel.findByIdAndUpdate(res.logId?.toString(), {
            responseBody: response,
            responseStatus: this.status,
            responseMessage: this.message,
            resposneHeaders: res.headers
        }, { new: true })
...
export class SuccessResponse<T> extends ApiResponse {
    constructor(message: string, private data: T, private pagination?: T) {
        super(StatusCode.SUCCESS, ResponseStatus.SUCCESS, message);
    }

    send(res: any): Response {
